...
### 问题描述
Skill 在执行过程中可能会在 `skills/img2Code/temp` 目录下产生临时文件（如中间态的分析草稿、临时图片等），若不清理会占用空间并可能造成混淆。

### 解决方案
- **清理时机**：在确认前端项目（页面）生成完毕且验证无误后执行。
- **执行操作**：
  1. 检查 `skills/img2Code/temp` 目录是否存在。
  2. 若存在，递归删除该目录及其下所有文件。
  3. 确保不删除 Skill 的核心配置文件（如 `SKILL.md`、`references/`）及 `assets/` 中的原始设计稿。
