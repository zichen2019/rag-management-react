...
- **Main Content (主内容区)**: 位于 Header 下方，浅灰色背景 `#f0f2f5`，内部包含上下两部分布局：
...
- **Main Content**: 占据剩余高度和宽度，内边距 24px。
- **标题区域**: 位于主内容区顶部，与下方内容间距 24px。
- **左侧主内容区**: 占据剩余宽度（flex: 1），包含统计卡片、表格、趋势图。
- **右侧面板**: 宽度 320px，与左侧主内容区间距 16px。
- **统计卡片**: 在左侧主内容区内占据整行宽度，4列布局，gutter 16px。
...
- **右侧**:
  ...
  - 通知图标:
    ...
    - 徽标: 数字 22，背景 `#ff4d4f`，文字白色，字号 9px，最小宽度 16px，高度 16px，圆角 8px，内边距 0 4px。
...
**布局**: 
- 4列等宽布局，`gutter: 16px`。
- 列之间使用垂直分割线分隔（`border-right: 1px solid #f0f0f0`，最后一列无）。
- 整体容器背景 `#ffffff`，圆角 8px，阴影 `0 1px 2px rgba(0,0,0,0.03)`。
- **响应式规约**:
  - `xl` / `lg` (≥ 992px): 4 列布局，保持分割线。
  - `md` (≥ 768px): 2 列布局（每行 2 个），移除分割线，改为 `gutter: 16px`，每列变为独立卡片（添加阴影+圆角）。
  - `sm` / `xs` (< 768px): 1 列布局，`gutter: 16px`，独立卡片样式。
...
- 内边距: 24px
- 悬停阴影: `0 2px 8px rgba(0,0,0,0.08)`（仅响应式拆分后生效）
...
─────────────────────────────────────┘
```
- **图标与文字区域**: Flex 横向排列，`align-items: center`，间距 16px。
- **文字区域**: Flex 纵向排列，`justify-content: center`。
...
─────────────────────────────────────┘
```
- **文字区域**: Flex 纵向排列，间距 4px。
- **分隔线**: `1px solid #f0f0f0`，距文字区域 8px，距图表 8px。
- **图表区域**: 高度 60px。
...
### 7. 右侧内容区 (RightContent)

**文件**: `src/components/KnowledgeBase/RightContent.tsx`

**区域样式**:
- 宽度: 320px
- 背景: `transparent`（内部卡片背景为 `#ffffff`）
- 与左侧主内容区间距: 16px
...
- 环形进度图:
  ...
  - 背景弧: `#f0f0f0`
  - 线条宽度: 8px
  - 中心文字: "85"（20px，600，`#262626`）+ "良好"（12px，`#8c8c8c`）
- 明细列表:
  - 布局: 三行，每行左侧圆点 + 文字
