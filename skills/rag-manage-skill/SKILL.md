...
3. **分层生成**: 按 `Layout` → `TitleBar` → `StatCards` → `KnowledgeTable` → `TrendCharts` → `RightContent` 顺序生成。
...
* **Right Content**: 宽度固定 `320px`，背景透明，内部包含独立白色卡片，与左侧主内容区 Flex 布局，间距 `16px`。
* **StatCards 布局**: 默认 4 列无间距 (gutter 0)，使用分割线，整体白色背景。宽度缩小时变为独立卡片。
...
* **Header Search**: 使用 `Input`，背景 `#ffffff`，边框 `1px solid #d9d9d9`，圆角 4px。图标位于右侧 (`suffix`)。
* **Notification Badge**: `Badge` 组件，徽标尺寸 16x16，字号 9px，偏移 `offset: [0, 0]`。左边距 24px，右边距 16px。
...
* **主内容区结构**: 
  ...
  * **下部左右布局**: 
    ...
    * **左侧主内容区**: `flex: 1; minWidth: 0;` (必须设置 minWidth: 0 防止内容溢出挤压右侧)。
    * **右侧内容区**: `width: 320px; flexShrink: 0;` (固定宽度，禁止收缩，背景透明)。
...
* **Content**: 上部标题区 + 下部 Flex 布局。左侧 Main Content (Flex 1)，右侧 Right Content (320px)。

### Level 2: StatCards (统计卡片)
* **布局**: 4 列等宽，`gutter: 0`，列间分割线，整体白色背景/圆角/阴影。占据整行宽度。
* **响应式**: `< 992px` 变为独立卡片 (gutter 16px)。
...
* **布局**: 4 列等宽，`gutter: 16px`。响应式断点同 StatCards。
...
### Level 5: RightContent (右侧内容区)
* **结构**: 透明背景，纵向排列 3 个独立白色卡片 (间距 16px)。
...
- [ ] 右侧内容区宽度是否为 320px？文件图标是否为单字母配色？
- [ ] 统计卡片是否占据整行宽度？宽度缩小时是否变为独立卡片？
